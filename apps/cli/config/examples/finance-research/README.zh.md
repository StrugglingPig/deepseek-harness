...
Patch 会把插入的 `finance-research` 行设置为 `provider: http`。它需要能够访问这些公共 Provider 端点。HTTP Provider 还会注册 `finance_provider_describe` 和 `finance_provider_request`；可在 Binance Spot/USD-M/COIN-M/Options、Yahoo、Polymarket Gamma 和 Polymarket CLOB base 下查询任意公开路径。数据可用性取决于上游 API 和凭据，而不是本地白名单。
