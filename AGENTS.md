...
- **In-box bare names resolve installation-authoritative from the launcher's anchor closure** (app plus composed bundles, tried in order); only names no anchor carries fall back to the config directory, and only on a not-carried code — a load failure at an owning anchor propagates loud, because two loaded copies split module identity ([contract](packages/boot/app-boot/README.md)).
