...
- Launcher-only glue is invisible to in-process scaffolds: guards, watchers, or resolution anchors only the published launcher wires (`installBundleLayerGuard`, `dshInstallAnchors`) never execute in scaffold e2e. Cover them with spawned-bin e2e, or install the glue into the scaffold and assert the protected behavior (`apps/cli/tests/web-agent-presets.e2e.ts`).
