...
- 仅启动器接线的胶水对进程内 scaffold 不可见：只有已发布启动器才会接入的 guard、watcher 或解析锚点（`installBundleLayerGuard`、`dshInstallAnchors`）在直接启动树的 scaffold e2e 中从不执行。用 spawn 构建产物的 e2e 覆盖它们，或把胶水显式装入 scaffold 并断言其所保护的行为（preset 组合的工具在重复行 guard 下存活，`apps/cli/tests/web-agent-presets.e2e.ts`）。
