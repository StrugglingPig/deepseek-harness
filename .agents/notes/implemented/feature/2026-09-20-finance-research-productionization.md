...
Mainland stock data uses a bundled Python bridge executed through `ctx.subprocess`. AKShare and both iFinD transports stay outside the npm runtime; the bridge detects missing dependencies and returns stable structured errors. The iFinD HTTP refresh token and local SDK account/password are resolved from `ctx.credentials` and passed only as explicit child environment variables. Stock tools cover normalized history, current quotes, and deterministic technical analysis without exposing Python traces or credentials to the model.
