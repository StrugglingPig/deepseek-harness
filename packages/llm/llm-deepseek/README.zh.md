...
- **文本路径下，序列化会将 user 与工具结果内容展平为文本块**：会跳过插件添加的块类型，空工具输出会以字面 `(no output)` 通过协议发送。
...
- **`reasoning_content` 重放遵循 `Config.reasoningPassback`**：默认值 `last-assistant` 与 `api.deepseek.com` 一致——后者会在任一**非最新**的 assistant 消息携带该字段时返回 400；仅当网关通过对重放思维链做哈希来恢复上游 thinking signature 时才设为 `every-turn`。
