...
describe('request-level dynamic configuration', () => {
  ...
  it('re-registers the route in place when the captured retry policy changes, without an empty-registry window', async () => {
    ...
    ctx.on('llm/adapters-updated', () => {
      // The route set is the whole subject: a directory republish that leaves
      // the same routes registered is not a provider disappearing, so the
      // assertion is on routes and not on how many topology events fired.
    ...
    // Every observed set holds the route: no event ever caught the registry
    // between an empty publication and the replacement.
    expect(new Set(observed.flat())).toEqual(new Set(['deepseek-official']))
  ...
  it('withdraws and restores the route a configuration surface removes', async () => {
    ...
    await configurations.get(ctx)!.update({ disabled: true })
    ...
    // never touched.
    await configurations.get(ctx)!.update({ disabled: false })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'deepseek-official', name: 'DeepSeek' }])
    expect(ctx.llm.listConfigurableProviders()).toEqual([{
  ...
  it('starts disabled without registering a route it was never allowed to hold', async () => {
    ...
    await configurations.get(ctx)!.update({ maxTokens: 4_096 })
    expect(ctx.llm.listProviders()).toEqual([])
