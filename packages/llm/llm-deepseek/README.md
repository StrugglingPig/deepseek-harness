...
The plugin offers `ctx.llm.registerModelDiscovery('llm-deepseek', …)`, answering "which models can this provider serve?" for a route or endpoint a configuration surface is editing. Like the pi-ai twin it is deliberately *not* a catalog refresh: nothing is stored, and the reply is candidates the surface offers for adoption. `settings.yaml` remains the only thing that decides what the route serves.

A request naming the owned route **without a base URL** is answered from the live advisory catalog (`models`, defaulting to V4 Flash and V4 Pro) with no network call; the answer reads the resolved settings snapshot per ask, so a settings-edited catalog is reflected without re-registration. A request naming a base URL — including one for the owned route — interrogates that endpoint through the shared OpenAI-compatible `GET /models` reader in `@deepseek-ai/dsh-llm` (`fetchOpenAiCompatibleModels`), which is the case a custom gateway or proxy is being tested for; a request naming neither the owned route nor an endpoint is told to set a base URL or enter the models by hand (`DISCOVERY_FAILED`). The reply is then matched by id against the live catalog: names and capacities the endpoint omitted are inherited from it (`enrichDiscoveredModels`), so an adopted row matches the shipped one; whatever the endpoint disclosed stays authoritative. The knowledge source is the live catalog merged with a per-id facts table: a built-in market-common default (every figure mirrors the vendor catalog data bundled with the pi-ai twin) overridden id by id by the `modelFacts` settings field, so a deployment whose proxy renames or adds models still auto-fills their capacities; the shipped advisory catalog additionally carries each V4 route's output cap.

A typed key wins; without one the interrogation resolves the section's `apiKeyEnv` through the credentials seam and sends it. A section with no stored key is probed unauthenticated rather than reporting `MISSING_CREDENTIAL` — a missing key is a posture under test, not a failure to report — while a stored key no HTTP header can carry is still refused as `INVALID_CREDENTIAL`.
...
- **Serialization flattens user and tool-result content to text blocks on the text-only path** — plugin-added block types are skipped, and empty tool output crosses the wire as the literal `(no output)`.
...
- **Replayed assistant tool-call arguments are repaired to a valid JSON object** — the gateway parses each `tool_calls[].function.arguments` to pair the call with the following `role:'tool'` messages and drops a call it cannot parse, which then rejects the paired tool message with `INVALID_REQUEST`; a raw arguments string that is not one JSON object (a turn can paste two objects back-to-back) is therefore replayed as `{}`. The repair is wire-only: the session log keeps the raw arguments and the model still receives the tool's own error result.
- **`reasoning_content` replay follows `Config.reasoningPassback`** — default `last-assistant` matches `api.deepseek.com`, which 400s when an earlier assistant carries the field; set `every-turn` only for a gateway that hashes the replayed chain of thought to recover the upstream thinking signature.
