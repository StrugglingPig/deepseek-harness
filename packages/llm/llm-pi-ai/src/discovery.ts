 ...
 * self-hosted server — is interrogated over the wire, through the shared
 * OpenAI-compatible listing reader ({@link fetchOpenAiCompatibleModels}) in
 * `@deepseek-ai/dsh-llm`, which `llm-deepseek` shares for its custom base URLs.
 *
...
import { fetchOpenAiCompatibleModels, LlmError } from '@deepseek-ai/dsh-llm'
 ...
 *   the network. A configuration surface never holds a stored secret — it
 *   edits a redacted descriptor — so without this an already-configured route
 *   would be interrogated unauthenticated and answer 401.
 * @returns the advertised models in endpoint order.
 * @throws LlmError when the protocol has no readable listing, the endpoint
 *   refuses or fails the request, or the reply is not a model listing;
 *   `INVALID_CREDENTIAL` for a blank or header-illegal probe key, and
 *   `ABORTED` on caller cancellation (both from the shared reader).
 */
