...
The generated [configuration catalog](../../../docs/config-catalog.md#deepseek-aidsh-experimental-finance-research) is the exhaustive field reference. Sixteen `valuation*` fields carry the equity valuation's analyst views and model structure: the projection horizon, the terminal growth rate, the equity risk premium, the risk-free fallback, the credit spread, the fallback tax rate, the bear and bull growth and margin shifts, the two scenario probabilities, the two action thresholds, and the terminal-value ceiling. They default in the schema and the panel edits them at runtime.

### Equity valuation

An equity report values the instrument from the statements it already loaded. The valuation section prints a model reference range with the implied upside at both ends rather than one number: an FCFF discounted cash flow over an explicit forecast, a fade period, and a terminal value; an earnings-power value that capitalises current profit with no growth; and a reverse DCF that solves for the growth the current price implies. The first screen carries a verdict box with the action, the credibility grade, the range, the weighted point, the implied upside, and the one-to-three-month trading direction; a football-field range draws every method on one axis with the current price marked; and a tearsheet lists the sourced figures with their sources and times. Bull, base, and bear paths carry explicit probabilities, the probability-weighted point prints beside the range, and a discount-rate table shows the value at the cost of capital plus and minus one percentage point. An assumption table names every parameter by the config field that sets it.

One cost of capital serves the whole report, built in one order from the ten-year yield the macro context loaded, the equity risk premium, the published beta, the after-tax cost of debt, and market-value weights; every component prints with its source and the time it was read. The earnings-quality battery — accruals ratio, cash conversion, and the Altman Z-score — grades the result A to D, a C grade caps the action at watch, and a D grade suppresses the range. A report may call its value a target price only after the recorded backtest passes. the tracked runner `.artifacts/backtest-valuation.ts` reconstructs symbol-dates from the endpoints this package already reads — the newest 10-K filed at least ninety days before each as-of date, that day's close, the share count the filing published, and the ten-year yield observed then — scores the model against an unchanged-price control on twelve-month absolute error, and runs a Diebold-Mariano test on the squared-error loss differential with the Harvey-Leybourne-Newbold correction and the shared-horizon covariance. `src/validation-record.ts` holds the latest recorded run, the report prints that result under its range, and until the recorded run clears the sample floor, the error comparison, and the significance test the output stays a model reference value.
...
- **Blocked upstreams are a transport concern, not a package setting** — the launcher installs one outbound proxy policy per process before plugins mount, so a Yahoo, CoinGecko, or CoinMarketCap address this network blocks is routed by that policy rather than by anything here; set `HTTPS_PROXY` in `~/.dsh/.env`, and note that the launcher refuses proxy variables in a repository `.env` so a checkout cannot redirect egress.
...
- **Each upstream contributes the free endpoints a report reads** — Finnhub supplies the US equity context described below; CoinGecko supplies the coin's community, developer, all-time-high and all-time-low changes plus the global market snapshot; GitHub supplies repository counts, trailing commits, and the trailing-year release feed; FRED supplies the macro series, including the commodity benchmarks and currency legs the supply-demand and FX sections render; EIA supplies the weekly US crude and natural-gas inventories behind an `FINANCE_EIA_API_KEY`; CFTC supplies the weekly Commitments of Traders non-commercial net position for WTI, gold, copper, the dollar index, the euro, and the yen without a key; Binance supplies spot klines; CoinMarketCap supplies quotes and OHLCV; Yahoo supplies the equity price history; Polymarket supplies prediction markets; AKShare and iFinD supply the A-share fields.
- **Blocked upstreams leave a marker, not an error** — Binance USD-M and COIN-M futures endpoints answer HTTP 451 from this network, Yahoo serves only the chart endpoint without a session crumb, and the AKShare holder endpoints time out, so a section that depends on them renders the data it has and lists the inputs it still lacks.
- **A section names the inputs it still lacks** — when a section renders part of what it needs, it prints the specific gaps (`energy inventories and the supply-demand balance`, `the cost curve and futures term structure`, `CFTC positioning`, `index valuation and earnings`, and so on) instead of a generic placeholder, so an unconfigured EIA key or a missing upstream is visible in the report itself.
- **The recorded backtest decides the wording** — the report names its value a target price only when the recorded run beats the unchanged-price control across at least fifty symbol-dates and the Diebold-Mariano p-value clears five percent. The latest run is recorded in `src/validation-record.ts`, it prints under the range, and a run that fails keeps every report on the model-reference wording.
- **The valuation prints a reference range, not a target price** — the FCFF DCF, the earnings-power value, and the reverse DCF run on the reported statements and the loaded yield, and the report says so beside every figure. Until an out-of-sample backtest reports its error and hit rate against an unchanged-price control, the output stays a model reference value and the report never calls it a target price or fair value.
- **A valuation input the filing does not publish prints as not obtained** — segments, consensus estimates, guidance, and a historical multiple series are not available from the free endpoints, so the report lists the ratio or signal it could not compute instead of substituting one.
- **US equity context needs a Finnhub key** — enable it and store the key in Finance settings; one US equity lookup reads the free tier's company profile, metric set, peers, company news, earnings calendar, EPS surprises, insider sentiment and insider transactions, analyst rating trends, SEC filings, and reported statements. The reported statements carry the income statement, the balance sheet, and the cash flow statement into the financial-quality block, and the other answers fill the valuation, competitive-position, ownership-and-insiders, and catalyst blocks. Eleven calls per lookup fit the free tier's 60 requests per minute; without the key a US equity report keeps its price, macro, and technical sections.
- **Market-wide Finnhub endpoints stay unwired** — symbol lookup, market status and holidays, and the general market news feed are free but describe the market rather than the company, so a per-company lookup does not request them, and no finance tool consumes them yet.
...
- **The Web dashboard is a separate plugin** — the dashboard reads `/api/finance-dashboard/market` from Connection's authenticated exact-route registry and renders crypto, A-share, and US-equity charts through `lightweight-charts`. A US-equity answer also carries the model research summary — the reference range, the credibility grade, the pre-registered action, the next earnings date, and the ratio table — which the panel prints beside the chart; a research read that fails leaves the chart mounted without the strip. The route registers only when the composition provides Connection, and private account data remains Host-only.
