...
One cost of capital serves the whole report, built in one order from the ten-year yield the macro context loaded, the equity risk premium, the published beta, the after-tax cost of debt, and market-value weights; every component prints with its source and the time it was read. The earnings-quality battery — accruals ratio, cash conversion, and the Altman Z-score — grades the result A to D, a C grade caps the action at watch, and a D grade suppresses the range. A report may call its value a target price only after the recorded backtest passes. `scripts/backtest-valuation.ts` in the repository's `.artifacts/` scratch area reconstructs symbol-dates from the endpoints this package already reads — the newest 10-K filed at least ninety days before each as-of date, that day's close, the share count the filing published, and the ten-year yield observed then — scores the model against an unchanged-price control on twelve-month absolute error, and runs a Diebold-Mariano test on the squared-error loss differential with the Harvey-Leybourne-Newbold correction and the shared-horizon covariance. `src/validation-record.ts` holds the latest recorded run, the report prints that result under its range, and until the recorded run clears the sample floor, the error comparison, and the significance test the output stays a model reference value.
