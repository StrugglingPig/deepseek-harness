...
One cost of capital serves the whole report, built in one order from the ten-year yield the macro context loaded, the equity risk premium, the published beta, the after-tax cost of debt, and market-value weights; every component prints with its source and the time it was read. The earnings-quality battery — accruals ratio, cash conversion, and the Altman Z-score — grades the result A to D, a C grade caps the action at watch, and a D grade suppresses the range. Paragraph text avoids the words target price and fair value: the report only earns them after an out-of-sample backtest, which has not run yet.
