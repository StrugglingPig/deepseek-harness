...
- **Fallback sources cover the two single-source fields** — US equity bars fall back to Alpha Vantage when Yahoo fails, and crypto market rows fall back to CoinGecko when CoinMarketCap fails; A-share fundamentals fall back to the Tonghuashun table when the indicator table is unavailable.
- **US fundamentals need an Alpha Vantage key** — enable it and store the key in Finance settings; the free tier allows 25 requests per day, and without it a US equity report keeps its price, macro, and technical sections.
