...
- **每个上游只接入报告真正读取的免费接口** — Finnhub 提供下文列出的美股上下文；CoinGecko 提供币种社区与开发数据、距历史最高/最低点变化以及全市场快照；GitHub 提供仓库计数、近 4 周提交和近一年发版记录；FRED 提供宏观序列，其中包含供需平衡与汇率章节要用的商品基准和汇率腿；Binance 提供现货 K 线；CoinMarketCap 提供行情与 OHLCV；Yahoo 提供股票价格历史；Polymarket 提供预测市场；AKShare 与 iFinD 提供 A 股字段。
