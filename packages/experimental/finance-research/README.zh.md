...
全报告只使用一个资本成本，按固定顺序构建：宏观上下文取到的 10 年期国债收益率、股权风险溢价、已发布 Beta、税后债务成本与市值权重；每个组成项都标注来源与读取时间。盈利质量三项（应计比率、现金转换、Altman Z 值）把结果分为 A–D 级，C 级把动作上限压到观察，D 级直接不输出区间。正文不使用“目标价”与“公允价值”：只有在样本外回测通过之后才启用这两个词，而回测目前还没有跑。
