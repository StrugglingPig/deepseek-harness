...
股票报告用已经取到的三张报表给标的估值。估值章节输出的是一条模型参考价值区间，两端都给出对应上行空间，而不是单一数字：显式预测年 + 递延年 + 终值的 FCFF 现金流折现；按当前盈利水平零增长资本化的盈利能力价值（EPV）；以及反解当前价格隐含增速的反向 DCF。牛市、基准与熊市三条路径各自带明确概率，概率加权点值与区间并列，贴现率表给出资本成本上下各 1 个百分点的对应价值。
