...
生成的 [configuration catalog](../../../docs/config-catalog.zh.md#deepseek-aidsh-experimental-finance-research) 是完整字段参考。16 个 `valuation*` 字段承载股票估值的分析师观点与模型结构：预测年限、终值增速、股权风险溢价、无风险利率兜底值、信用利差、兜底税率、熊市与牛市的增速与利润率偏移、两个情景概率、两个动作阈值以及终值占比上限。它们在 schema 中有默认值，并可在面板中实时修改。

### 股票估值

股票报告用已经取到的三张报表给标的估值。估值章节输出的是一条模型参考价值区间，两端都给出对应上行空间，而不是单一数字：显式预测年 + 递延年 + 终值的 FCFF 现金流折现；按当前盈利水平零增长资本化的盈利能力价值（EPV）；以及反解当前价格隐含增速的反向 DCF。首屏给出结论框：动作、可信度等级、参考区间、加权点值、对应上行空间以及 1–3 个月方向；方法区间图把各方法画在同一条轴上并标出现价；关键数据表列出每个数字的来源与读取时间。牛市、基准与熊市三条路径各自带明确概率，概率加权点值与区间并列，贴现率表给出资本成本上下各 1 个百分点的对应价值，假设表按配置字段名列出每个参数。

全报告只使用一个资本成本，按固定顺序构建：宏观上下文取到的 10 年期国债收益率、股权风险溢价、已发布 Beta、税后债务成本与市值权重；每个组成项都标注来源与读取时间。盈利质量三项（应计比率、现金转换、Altman Z 值）把结果分为 A–D 级，C 级把动作上限压到观察，D 级直接不输出区间。报告只有在已记录的回测通过之后，才会把自己的数值称为目标价。仓库中已提交的脚本 `.artifacts/backtest-valuation.ts` 用本包已经读取的接口重建“标的×日期”样本：每个 as-of 日期之前至少 90 天已提交的最新 10-K、当天的收盘价、该财报披露的股本数，以及当时观测到的 10 年期国债收益率；随后用 12 个月平均绝对误差与“价格不变”控制组对比，并对平方误差损失差做 Diebold-Mariano 检验（含 Harvey-Leybourne-Newbold 小样本修正与重叠视界协方差）。最新一次回测记录在 `src/validation-record.ts`，报告会在区间下方打印该结果；在记录跑赢样本量下限、误差对比与显著性检验之前，输出始终是模型参考价值。
...
- **每个上游只接入报告真正读取的免费接口** — Finnhub 提供下文列出的美股上下文；CoinGecko 提供币种社区与开发数据、距历史最高/最低点变化以及全市场快照；GitHub 提供仓库计数、近 4 周提交和近一年发版记录；FRED 提供宏观序列，其中包含供需平衡与汇率章节要用的商品基准和汇率腿；EIA 在配置 `FINANCE_EIA_API_KEY` 后提供美国原油与天然气周度库存；CFTC 无需密钥即可提供 WTI、黄金、铜、美元指数、欧元与日元的周度非商业净持仓；Binance 提供现货 K 线；CoinMarketCap 提供行情与 OHLCV；Yahoo 提供股票价格历史；Polymarket 提供预测市场；AKShare 与 iFinD 提供 A 股字段。
- **被拦截的上游只留标记，不报错** — 本网络对 Binance 交割/永续合约接口返回 HTTP 451，Yahoo 除 chart 之外的接口需要会话 crumb，AKShare 股东接口会超时；依赖这些数据的章节会渲染已有数据并列出仍缺的输入。
- **章节会列出仍缺的具体输入** — 只拿到部分数据时，章节会写出具体缺口（`能源库存与供需平衡表`、`成本曲线与期限结构`、`CFTC 持仓`、`指数估值与盈利` 等），而不是笼统一句提示，这样未配置的 EIA Key 或缺失的上游在报告正文里就能看到。
- **用词由已记录的回测决定** — 只有在“标的×日期”样本不少于 50 个、且 Diebold-Mariano p 值低于 5% 并跑赢“价格不变”控制组时，报告才会把数值称为目标价。最近一次结果记录在 `src/validation-record.ts` 并打印在区间下方；未通过时所有报告保持“模型参考价值”的措辞。
- **估值输出的是参考区间，不是目标价** — FCFF 现金流折现、盈利能力价值与反向 DCF 都基于已披露报表和已加载的国债收益率，报告在每个数字旁标明这一点。在样本外回测给出误差与命中率（并和价格不变的控制组比较）之前，输出始终是模型参考价值，报告不会称其为目标价或公允价值。
- **报表没有披露的估值输入会标为未获取到** — 分部数据、市场一致预期、管理层指引与历史估值倍数序列都不在免费接口范围内，因此报告会列出无法计算的比率或信号，而不是用替代值填充。
- **美股上下文数据需要 Finnhub Key** — 在金融设置中打开开关并保存密钥；一次美股查询会读取免费档的公司概况、指标集、同业公司、公司新闻、财报日历、EPS 超预期、内部人情绪与内部人交易、分析师评级趋势、SEC 公告以及已披露财报。已披露财报把利润表、资产负债表和现金流量表带进财务质量章节，其余回答用于填充估值、竞争格局、股权与内部人、催化剂章节。单次查询共 11 次请求，符合免费档每分钟 60 次限额；未配置密钥时美股报告保留价格、宏观与技术面章节。
