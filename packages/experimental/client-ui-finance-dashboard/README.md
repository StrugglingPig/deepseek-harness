...
The header selects the asset family, symbol, and interval and exposes an explicit refresh. Tabs cover crypto pairs, mainland A-shares, and US equities, each with a quick-symbol list. Metrics show the latest price, the change against the previous bar, and the latest bar volume. A US-equity snapshot also carries the Host's research summary: the model reference range with its probability-weighted value, the credibility grade, the pre-registered action, the next earnings date, and the ratio table, with a ratio the statements cannot support printed as not obtained rather than blank. The chart renders the selected indicators through `lightweight-charts`, with an inline SVG line when canvas is unavailable. The indicator settings module groups the overlays and the separate panes, toggles each indicator, edits its periods, and restores one indicator or all of them to the shipped defaults; the selection and parameters persist in browser storage under `dsh.finance.dashboard.indicators.v1`. TD Sequential marks its buy and sell setups with counts on the candles and highlights counts at the configured target. Every pane captions the indicator it draws, and the price pane lists the overlays in place. A symbol without history shows an empty panel carrying a refresh action instead of the provider failure text, which stays behind that panel's technical-details disclosure. The request badge reports connecting, live, or error state; the controller re-polls every 15 seconds, stops polling when the panel is disposed, and drops responses that arrive after the selection changed.
