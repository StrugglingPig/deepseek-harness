...
entry 集合变化——宿主上一个插件行的出现或消失，例如市场安装活重组了 profile 的 bundle 层——无法走上述原位替换，因此 node 侧还订阅 client 表的 `onGraphChanged`，每次变更向所有已连接的事件客户端写入一个新的 `graph` 帧（纯内容重建仍走 `rebuilt`）。浏览器侧把每帧的 id 集合与页面启动 manifest（`window.__DSH_BOOT__`）比较：集合相同则取消待定 reload；集合不同则在短暂防抖后调度一次整页 reload，触发时再核一次，若后续帧恢复了启动集合则取消，使新的 manifest 物化该安装或卸载。没有启动 manifest 的页面永不因帧 reload。
...
- **entry 集合变化会 reload 整个页面**：一次活的安装/卸载与手动刷新一样丢失页内插件状态；免 reload 的增量挂载/卸载是同一 `graph` 帧协议的后续扩展。
