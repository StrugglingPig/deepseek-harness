/**
 * The drafted model rows the Models settings card edits: the structurally open
 * record shape, the K/M capacity vocabulary, and the adapter constraints the
 * serialized schema cannot express. Pure — no component state, no wire.
 * @module @deepseek-ai/dsh-client-ui-settings-models/model-drafts
 */
...
export function parseCapacity(text: string): number | undefined {
  ...
  const match = trimmed.match(CAPACITY_PATTERN)
 ...
 * @param value - user-owned `models` value, or anything a hand-edited settings document held.
 * @returns one open record per array entry; non-object entries become empty
 *   records. Rows alias the input entries; callers must not mutate them in place.
 */
