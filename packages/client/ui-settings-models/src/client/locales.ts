...
export const en = {
  ...
  deleteDescriptionBuiltIn: 'Deleting {provider} removes its saved configuration and stored API key, and stops serving it. Adding it again starts from the built-in defaults, so enter the key and endpoint once more.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  deleteDescriptionBuiltIn: '删除 {provider} 会清除已保存的配置和存储的 API 密钥，并不再使用它。重新添加时使用内置默认值，需要重新填写密钥与地址。',
