...
export const en = {
  ...
  deleteDescriptionBuiltIn: 'Deleting {provider} stops serving it to this harness. Nothing is erased: add the provider again to restore it with these settings.',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  deleteDescriptionBuiltIn: '删除 {provider} 后本机不再使用它，不会清除任何设置：在「添加提供方」里重新添加即可原样恢复。',
