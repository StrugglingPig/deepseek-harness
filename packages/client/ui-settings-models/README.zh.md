...
提供方的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口与输出上限收在该行的展开区内，右侧是两个无文字的操作——展开与删除。两个设置家族都通过这同一个编辑器编辑各自的行：pi-ai profile，以及 DeepSeek 卡片——后者的 profile `defaultContextWindow` 与 `maxTokens` 定出容量占位符的量级，pi-ai 则保留通用量级。对 pi-ai，空列表意味着「使用该路由的内置 catalog」；在 DeepSeek 卡片上，显式空列表是空的建议目录，内置默认仅经恢复返回——因此每一行都只会被刻意添加。清空容量会丢弃它，而不是存入一个 schema 会拒绝的值。不是正整数的容量根本不会被存下。

**获取可用模型**会针对表单**当前显示**的端点调用 `llm.discoverModels`，包括已修改但尚未保存的 API 地址和已键入但尚未存储的密钥，因此新增一个提供方是一趟走完，而不是「先保存再回来」。两个家族都提供它：pi-ai profile 在其 catalog 未描述该路由时询问它指名的端点；DeepSeek 卡片在未填端点时询问其适配器的活建议目录——自有路由无需网络——而填入的 API 地址则通过同一个共享读取器询问该代理。回复会打开一个选择框而不是直接写入：已配置过的候选默认不勾选，因此采纳一次选择绝不会覆盖用户已更正的容量。**全选**／**取消全选**只会改变选择框当前的勾选状态；点击**添加所选**之前不会写入任何内容。被采纳的行以模型 id 作为显示名称默认值；端点未提供名称或容量时，host 会按 id 从适配器 catalog 继承，因此即便经由朴素列表获取的行，读起来也与内置行一致。无法被询问的提供方只是绕路而非死路——适配器自己的消息会显示在各行旁边，而这些行仍可手工编辑。
