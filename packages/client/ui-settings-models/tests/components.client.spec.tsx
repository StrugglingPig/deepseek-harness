...
const DeepSeekConfig = Schema.object({
  // A schema-minted default reference: a whole-section provider that names
  // none resolves through this, which is how the page tells a key it wrote
  // from configuration it never touched.
...
describe('ModelsSection', () => {
  ...
  it('claims the schema default reference a whole-section provider resolves through', async () => {
    // DEEPSEEK_API_KEY is the reference the page resolves a whole-section
    // provider's key through, so a key stored under it is one this page wrote,
    // and Delete clears it with the section.
    const scripted = scriptedFace()
    scripted.face.credentials.describe.mockImplementation((refs: string[]) => Promise.resolve(remoteOk(
