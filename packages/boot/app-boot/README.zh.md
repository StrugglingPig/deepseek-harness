...
| `mountRootInclude(ctx, absoluteConfigPath, patches?, bareModuleAnchors?)` | 注册静态导入的 `cordis:include` 与 `cordis:group` builtin，挂载 include，并保留用户 patch 层 HMR（热模块替换）使用的确切根配置项；可选模块基准（安装侧 app 与每个组合包的 package.json）使裸包名按序优先从安装侧解析——单一 package.json 无法覆盖整个 in-box 闭包——树外名称回退到配置目录，相对名称仍以配置目录为基准 |
...
| `watchProfileManifest(ctx, options)` | 把 profile manifest（`package.json`）注册到相同的 HMR 接缝；每次改写（包管理器的安装/卸载）都通过调用方的 `compose` 闭包以事务方式重新应用完整层栈——被监听文件本身从不按补丁解析——在成功更新后运行可选的 `afterUpdate`，通过有界 `retry` 自愈尚未安装完成的 bundle 代，并以 stat 兜底（`pollMs`，默认 2s）调和原生监听事件被丢弃的改写；在某代运行中落下的请求会被合并为一次重跑，写入不会丢失；返回异步 disposer |
| `rootIncludeSubtree` / `duplicateSubtreeEntry` / `disposeDuplicateSubtreeEntries` / `installBundleLayerGuard` | bundle 层活化的热挂载调和：持久层顶层之外、按包名与存活持久行重复的临时行，在两种落地顺序下都让位（见 [Profile](#profiles)） |
| `resolveProfileDir` / `initProfile` / `loadProfile` / `reloadProfileLayers` / `resolveBundleLayers` / `readProfileManifest` / `writeProfileManifest` / `resolveBundleDir` / `composeEntries` / `healProfilesModuleFallback` / `PROFILE_TEMPLATES` / `DEFAULT_PROFILE_BUNDLES` / `PROFILES_DIR` / `PROFILE_PATCH_FILENAME` | Profile 机制（见 [Profile](#profiles)） |
| `boot(binName, absoluteConfigPath, patches?, prepare?, bareModuleAnchors?)` | 创建根上下文，向 Loader `!!js` 配置表达式暴露 `dshHomePath(...segments)` 并安装 Loader，在配置树条目挂载前执行可选的宿主准备操作（`prepare` 可以使用 Loader，也可以提供由启动器拥有的上下文插槽），再挂载并等待 include 树结算，断言所有条目均已加载并激活，最后返回根上下文——失败时 dispose（资源释放）部分构造的上下文，并以带标签的错误 reject；可选模块基准与 `mountRootInclude` 的解析语义相同 |
...
每个长驻 profile 启动还会通过 `watchProfileManifest` 让 bundle 层保持活化：包管理器在 `dsh plugin add/remove`（或任何其他 manifest 写入方）时改写的 profile `package.json`，会触发同样的事务性重应用——调用方的 `compose` 每代经 `reloadProfileLayers` 重新解析 bundle 层（它会先修复共享模块回退目录，且从不改写 `cordis.yml`），并重新读取各用户层。由于 manifest 可能先于包管理器物化列出 bundle，有界重试会按延时重跑该代，无需第二次文件事件；耗尽后经同样的 `hmr/config-update-failed` 广播暴露。原生事件投递也可能在包管理器大量 churn 下整体丢弃一次改写，因此 stat 兜底按间隔核对 manifest mtime，并重新触发同一条串行化世代路径；在某代运行中落下的请求会在其后合并为一次重跑——运行中的那代可能在写入落盘前就读了 manifest。移除 bundle 会通过同一次更新事务性地 dispose 其各行的资源。临时热挂载——挂在森林更深一层的 market 风格行，或位于另一个 loader realm 的行——必须让位于持久 bundle 层，无论二者谁后落地：`installBundleLayerGuard` 在被标记的行出现时移除它，`disposeDuplicateSubtreeEntries`（即 `afterUpdate` 钩子）调和先落地的副本。持久层恰好是根 include 子树的顶层——每份组合出的补丁都插入那里，成员判定读取该顶层 store，因为按森林递归会错误地庇护热挂载自己的嵌套行。只有该顶层之外、包名与存活且启用的持久行重复的、已创建条目会被标记；无 entry 的 fiber（preset 子树）以及没有同名存活持久行的条目永不被触碰（[理由](../../../.agents/notes/implemented/architecture/2026-08-19-live-profile-bundle-recomposition.md)）。
