...
Bare row names in a composition resolve through the launcher-provided `dshInstallAnchors` context slot (the installed app's and every bundle's package.json, tried in order) before the composition base's own `node_modules` walk, so a preset's in-box rows load the same package instances as the host tree even when a stale profile-hoisted copy sits in the walk; relative rows and names no anchor carries keep the composition base.
